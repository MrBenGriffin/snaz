...
//		}
	...
	void Text::doFor(mtext& result,const forStuff& stuff) const {
		...
		if(!stuff.valueToken.empty()) {
			while ((curr=text.find(stuff.valueToken,start)) != string::npos) {
				if(!stuff.countToken.empty()) {
//					substitute(basis,stuff.countToken,stuff.count,stuff.ctSize,curr);
				} else {
					basis.append(text.substr(start,curr - start));
				}
				basis.append(stuff.valueToken);
				curr  += stuff.vtSize;
				start += curr;
			}
		}
		if(start < text.size()) {
			basis.append(text.substr(start));
		}
