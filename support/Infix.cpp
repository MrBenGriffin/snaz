...
namespace Support {
	namespace Infix {
		...
		void Evaluate::startup() {
			...
			lut.insert(lut_t::value_type("mod",new modulo()));
		...
		void Evaluate::add_parm(string key, long double value) {
			parms.insert({key,value});
		}

		long double Evaluate::process(Messages& errs) {
			const string wss = "\n\r\t ";
			valstack.clear();
			opstack.clear();
				...
				if (curop == nullptr) {
					...
					if (!cur_name.empty()) {
						...
						if(x == '(') {
							...
							if ( curop == nullptr) {
								errs << Message(error,"Unknown function '" + cur_name + "'.");
							}
						}
					} else {
						char err = *i;
			...
			if (!valstack.empty()) {
				...
				errs << Message(error,"Stack Underflow Error.");
				return nanl("");
			}
		}
		...
		char Evaluate::evalstack(Messages& errs) {
			char retval = '?';
			if (!opstack.empty()) {
				...
				if (pop->parms <= valstack.size()) {
					...
					errs << Message(error,"Error. Stack underflow. Too few values.");
					valstack.push_back(nanl(""));
				}
			}
			return retval;
		}
		long double Evaluate::getvalue(string& str) {
			long double value(nanl(""));
			auto o = parms.find(str);
			if( o != parms.end() ) { value = o->second; }
			return value;
		}
...
//			return p >= q ? 1:0;
		...
		long double rolfn::evaluate(const long double o,const long double p,const long double q) const {
			auto x = (unsigned long long)o;	 //number to change
			auto b = (unsigned long long)(q > 0 ? q : sizeof(unsigned long long) << 3);	 //wordsize
			unsigned long long s = (long long) p % b;	//number of shifts to make is mod q.
			auto m = (unsigned long long)(pow(2,q) - 1);			//mask
